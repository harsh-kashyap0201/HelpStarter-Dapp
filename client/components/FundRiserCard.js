...
const FundRiserCard = ({props,pushWithdrawRequests}) => {
  ...
  return (
    <div className="card relative overflow-hidden my-4 bg-[#EDF2F4]">
    <div className={`ribbon ${colorMaker(props.state)}`}>{props.state}</div>
    <Link href={`/project-details/${props.address}`} >
      <h1 className="font-sans text-xl text-black font-bold hover:text-[#D80032] hover:cursor-pointer">{props.title}</h1>
    </Link>
    <p className="font-sans text-sm text-black text-stone-800 tracking-tight">{props.description}</p>
